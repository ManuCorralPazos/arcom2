...
int main(int argc, char **argv){
    ...
    dp=(struct quaternion*)malloc(hilos*sizeof(struct quaternion));
    ...
    for(vez=0; vez<10; vez++){
        start_counter();
        inicializarACero(dp, hilos);
        //for(int i=0; i<N; i++){
          //  *dp=operacionQuaternion(*dp, multiplicarQuaternion(a[i], b[i]));
        //}
        #pragma omp parallel private(i, tid) num_threads(hilos)
        {
            tid=omp_get_thread_num();
            #pragma omp for
            for(i=0; i<N; i+=5){
                dp[tid]=operacionQuaternion(dp[tid], multiplicarQuaternion(a[i], b[i]));
                dp[tid]=operacionQuaternion(dp[tid], multiplicarQuaternion(a[i+1], b[i+1]));
                dp[tid]=operacionQuaternion(dp[tid], multiplicarQuaternion(a[i+2], b[i+2]));
                dp[tid]=operacionQuaternion(dp[tid], multiplicarQuaternion(a[i+3], b[i+3]));
                dp[tid]=operacionQuaternion(dp[tid], multiplicarQuaternion(a[i+4], b[i+4]));
            }
        }
        for(i=0; i<hilos; i++){
            dp[0]=sumarQuaternion(dp[0], dp[i]); //dp[0]+=dp[i];
        }    
        ck[vez]=get_counter();
    }
